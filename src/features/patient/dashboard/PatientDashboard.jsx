...
export default function PatientDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setProfile(user);
        // Fetch appointments using the patient ID from profile
        if (user.patient) {
          const apptRes = await api.get(`/appointments/patient/${user.patient}`);
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-screen bg-gray-100"><Spinner /></div>;
  }
  if (error) {
    return <div className="flex items-center justify-center min-h-screen bg-red-50 text-lg text-red-700">{error}</div>;
  }
    ...
    <div className="min-h-screen bg-gray-100 py-8 px-4">
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <div className="bg-gradient-to-br from-blue-200 via-blue-100 to-blue-300 rounded-2xl shadow p-6 flex flex-col justify-center">
          <h1 className="text-2xl font-bold text-blue-900 mb-2">Welcome{profile?.fullName ? `, ${profile.fullName}` : ""}!</h1>
          <p className="text-gray-700 mb-4">This is your patient dashboard. Here you can view your profile and keep track of your appointments.</p>
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-2 text-gray-700">
              ...
              <div><span className="font-semibold">User ID:</span> {profile._id}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-2xl shadow p-6 border border-gray-200">
          <h2 className="text-lg font-semibold mb-4 text-gray-700">Recent Appointments</h2>
